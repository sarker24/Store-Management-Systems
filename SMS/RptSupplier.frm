...
Begin VB.Form RptSupplier 
   BackColor       =   &H00C0B4A9&
   Caption         =   "Supplier Report Statment"
   ClientHeight    =   3705
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   6465
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.CheckBox chkAllItem 
         ...
         Width           =   1455
      End
      Begin MSForms.ComboBox cmbSupplierName 
         Height          =   375
         Left            =   2160
         TabIndex        =   12
         Top             =   240
...
Dim rsTemp2 As New ADODB.Recordset
    ...
    While Not rsTemp2.EOF
        cmbSupplierName.AddItem rsTemp2("sName")
        rsTemp2.MoveNext
    Wend
    rsTemp2.Close
End Sub
...
Set rsMaster = New ADODB.Recordset
 ...
 rsMaster.Open "SELECT SerialNo, RDate, SName, StoreName, Catagory, ItemName," & _
               "Qty, Rate, Amount, Rol, ExpDate, Posted, Warranty, Remarks, CPost, Unit " & _
               "FROM SStockDetails where  RDate='" & SSCurrentDate.Value & "'And CPost = 'Posted'and SStockDetails.SName='" & parseQuotes(cmbSupplierName) & "'", cn, adOpenStatic, adLockReadOnly
 ...
 rsMaster.Open "SELECT SerialNo, RDate, SName, StoreName, Catagory, ItemName," & _
               "Qty, Rate, Amount, Rol, ExpDate, Posted, Warranty, Remarks, CPost, Unit " & _
               "FROM SStockDetails where RDate BETWEEN '" & SSFDate.Value & "' AND '" & SSTDate.Value & "'And CPost = 'Posted'and SStockDetails.SName='" & parseQuotes(cmbSupplierName) & "'", cn, adOpenStatic, adLockReadOnly
...
On Error GoTo ErrH
    ...
    End If
        ...
        strPath = App.Path + "\reports\Supplier wise Statement Summary.rpt"
   ...
   Set objReportFF = objReportFormulaFieldDefinations.Item(4)
              objReportFF.text = "'" + cmbSupplierName + "'"
...
On Error GoTo ErrH
    ...
    End If
        ...
        strPath = App.Path + "\reports\Supplier wise Statement.rpt"
        Set objReportApp = CreateObject("Crystal.CRPE.Application")
        Set objReport = objReportApp.OpenReport(strPath)
        Set objReportDatabase = objReport.Database
        Set objReportDatabaseTables = objReportDatabase.Tables
